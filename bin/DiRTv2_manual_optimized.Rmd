...
canonical_transcripts <- tran_length_table %>%
  ...
  pull(tx_name) %>%
  # Strip optional "transcript:" / "gene:" / "CDS:" prefix (present in
  # Ensembl-style GFFs, absent in others) so the match against the
  # post-strip exon parent IDs below works in both cases.
...
CDS.info.sorted <- gene.info.from.gff %>%
  ...
  # 1. Safely generate fallback columns. 
  # If the column doesn't exist in this specific file, it fills it with NA instead of crashing.
  mutate(
    gene_id = if("gene_id" %in% names(.)) gene_id else NA_character_,
    Parent  = if("Parent" %in% names(.)) Parent else NA_character_,
    ID      = if("ID" %in% names(.)) ID else NA_character_
  ) %>%
  mutate(universal_id = coalesce(gene_id, Parent, ID)) %>%
  dplyr::select(
    Chr = seqnames, 
    start, 
    end, 
    strand, 
    type, 
    gene.id = universal_id
  ) %>%
mutate(
    gene.id = str_remove(gene.id, "^(gene:|CDS:|transcript:)"),
    gene.id = str_remove(gene.id, "(\\.[tT]?\\d+.*|[-_][tT]\\d+.*)$")
  )
...
exon.info.canonical <- gene.info.from.gff %>%
  ...
  filter(gene.id %in% canonical_transcripts)%>%  mutate(
    gene.id = str_remove(gene.id, "^(gene:|CDS:|transcript:)"),
    gene.id = str_remove(gene.id, "(\\.[tT]?\\d+.*|[-_][tT]\\d+.*)$")
  )
...
# `samtools faidx` skipped (the .fai is provided via the `genome_fai` param
# and exposed to bash as $FAI_FILE in the setup-params R chunk).
# `conda activate` lines removed because they break a non-interactive bash
# chunk. Everything else is unchanged.
...
# Derive an implicit sample ID for each BAM by stripping params$bam_pattern from
# the basename. e.g. with bam_pattern = "\\.sorted\\.bam$" -> "RSL1.sorted.bam"
# becomes "RSL1"; with bam_pattern = "\\.sorted\\.merged\\.bam$" ->
# "ERR1.sorted.merged.bam" becomes "ERR1". Then match those derived IDs to
# params$sample_names so the column order is deterministic.
implied    <- sub(params$bam_pattern, "", basename(bamfiles))
match_idx  <- match(params$sample_names, implied)
if (any(is.na(match_idx))) {
  stop("Could not match all params$sample_names to BAM files in ", bam.dir,
       "\n  Expected sample IDs (params$sample_names): ", paste(params$sample_names, collapse = ", "),
       "\n  Derived from BAM basenames:                ", paste(implied, collapse = ", "),
       "\n  bam_pattern used to derive them:           ", params$bam_pattern,
       "\n  All BAMs found:                            ", paste(basename(bamfiles), collapse = ", "))
}
...
```{bash}
# 1. Safely process File A (Strip Windows \r, verify numbers, fix math, force tabs)
awk '{
    sub(/\r$/, ""); 
    if ($2 ~ /^[0-9]+$/ && $3 ~ /^[0-9]+$/) {
        if (($2+0) > ($3+0)) {
...
}' OFS='\t' One_intron.txt > One_intron_fixed.bed

# 2. Safely process File B
awk '{
    sub(/\r$/, ""); 
    if ($2 ~ /^[0-9]+$/ && $3 ~ /^[0-9]+$/) {
        if (($2+0) > ($3+0)) {
...
}' OFS='\t' ALL_introns_info.txt > ALL_introns_fixed.bed

# 3. Sort the safely generated files
bedtools sort -i One_intron_fixed.bed > One_intron_sorted.bed
bedtools sort -i ALL_introns_fixed.bed > ALL_introns_info_sorted.bed

# 4. Run the closest command
bedtools closest -a One_intron_sorted.bed -b ALL_introns_info_sorted.bed > One_Intron_closest_Intron.bed
```
...
}' OFS='\t' RNA_No_intron.txt > RNA_No_intron_fixed.bed
...
# 3. Sort the safely generated files
bedtools sort -i RNA_No_intron_fixed.bed > RNA_No_intron_sorted.bed
bedtools sort -i ALL_introns_fixed.bed > ALL_introns_info_sorted.bed

# 4. Run the closest command with your -k 2 flag
bedtools closest -a RNA_No_intron_sorted.bed -b ALL_introns_info_sorted.bed -k 2 > RNA_No_Intron_closest2_Introns.bed
```
...
# export results
```{r}
write_xlsx(DT_Final_Classified, path = file.path(params$out_dir, "Final_Result_RSR_toSHZ2alleles.xlsx"))
```
