---
title: "DiRTv2_Using_Gff_annotation"
...
```{r}
gene.info.from.gff <- rtracklayer::import("F:/PhD_Research/All_Annotations/Genome_annotation.gff3") %>%
  as.data.frame()
...
```Linux
tRNAscan-SE -E -o tRNA.txt Reference_genome.fasta
awk 'NR > 3 {print $1 "\t" $3 "\t" $4 "\t" $5 "\t" $9 "\t" $6}' tRNA.txt > tRNA.bed
```
# Read tRNA info
```{r}
tRNA.info <- read_delim("tRNA.bed", delim = "\t", col_names = FALSE) 
...
cut -f1,2 Reference_genome.fasta.fai > Reference_genome.txt
...
```{r}
Seqinfo <- read_delim("Reference_genome.fasta.fai", 
...
```{r}
#UTR information
Gff_tran_gr <- import("Reference_genome_annotation.gff3")
mcols(Gff_tran_gr) <- mcols(Gff_tran_gr)[, c("ID", "Parent", "type")]
Gff_tran_db <- txdbmaker::makeTxDbFromGRanges(Gff_tran_gr)
Gff_five_utrs  <- fiveUTRsByTranscript(Gff_tran_db, use.names = TRUE)
Gff_three_utrs <- threeUTRsByTranscript(Gff_tran_db, use.names = TRUE)

Gff_five_utrs_df <- as.data.frame(Gff_five_utrs)
Gff_three_utrs_df <- as.data.frame(Gff_three_utrs)
# Find the longest transcript for each gene
tran_length_table <- transcriptLengths(Gff_tran_db, 
                                  with.cds_len = TRUE, 
                                  with.utr5_len = TRUE, 
                                  with.utr3_len = TRUE)
...
# Filter 5' UTRs
Gff_five_utrs_longest <- Gff_five_utrs_df %>%
  filter(group_name %in% canonical_transcripts)
# Filter 3' UTRs
Gff_three_utrs_longest <- Gff_three_utrs_df %>%
  filter(group_name %in% canonical_transcripts)

five_prime_clean <- Gff_five_utrs_longest %>%
  as.data.frame() %>%
...
three_prime_clean <- Gff_three_utrs_longest %>%
  as.data.frame() %>%
...
```{r}
# specify Samples
bam.dir <- "/Path_to_bam_files/" # Dir for bamfiles
...
write.table(tRNA.counts,
            file = "tRNA_counts.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
### Load tRNA counts
```{r}
tRNA.counts <- read.delim("tRNA_counts.xlsx", stringsAsFactors = FALSE)
```
## filter commonly expressed tRNA 
```{r}
# filter expressed tRNA 
tRNA.expressed.AllSample <- rowSums(dplyr::select(tRNA.counts,6:8) >= 1) == 3 # change the column (6:8) and rowSums number (3) according to your replicates
...
write.table(gene.counts,
            file = "gene_counts.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```

### Load gene counts
```{r}
gene.counts <- read.delim("gene_counts.xlsx", stringsAsFactors = FALSE)
```
...
write.table(expressed.gene.only.one.exon,
            file = "tGene_expressed_no_intron.xlsx",
           quote = FALSE, sep = "\t", row.names = FALSE)

write.table(tgene.expressed.with2exon,
            file = "tGene_expressed_one_intron.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
# Get UTR information
```{r}
...
```
# pull out intron coordinates based on the exon information (WHY Charllot's old pipeline integrate UTR infor into intron???)
```{r}
...
DicistronicName <- names(coverage.intergenic$RNA.sorted.bam) 
...
IntronName <- names(coverage.intron.closest$RNA.sorted.bam)
...
IntronName <- names(coverage.intron.2nd.closest$RNA.sorted.bam)
...
```{r}
allIntergenic <- names(coverage.intergenic$RNA.sorted.bam) # pull out sample name from any sample
...
write.table(continuous.coverage.final,
            file = "Continuous_Coverage_with_two_intron.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
...
bedtools closest -a T2T_One_intron.txt -b ALL_introns_info.txt > One_Intron_closest_Intron.bed
```
...
IntronName <- names(coverage.intron.Self$RNA.sorted.bam) # same as DicistronicName
...
IntronName <- names(coverage.intron.Nearby$RNA.sorted.bam)
...
tRnaToGene_One_intron <- One_Intron_aveCoverage.final %>%
  ...
  split(f = .$id) %>%
  lapply(function(x){
    if (nrow(x) == 1) return(x) # This removes the genes with >1 nearby tRNA, already fixed with unique tRNA-gene combo id
  }) %>%
  bind_rows() 
```
...
coverage.list.final_One_intron <- coverage.list.final_One_intron %>%
  dplyr::select(ids,
                RNA1.sorted.bam_nZero,
                RNA2.sorted.bam_nZero,
                RNA3.sorted.bam_nZero) # replicates 1 to 3
One_Intron_Potential_DT_Cantdidates <- rowSums(dplyr::select(coverage.list.final_One_intron, 2:4) == 0 ) == 3 
...
write.table(continuous.coverage.final_One_intron,
            file = "Continuous_coverage_with_one_intron.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
...
IntronName <- names(coverage_No_Intron_1st_Nearby$RNA.sorted.bam) 
...
IntronName <- names(coverage_No_Intron_2st_Nearby$RNA.sorted.bam) 
...
write.table(continuous.coverage.final_No_intron,
            file = "Continuous_coverage_with_No_intron.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
...
```{r}
continuous.coverage.final.all <- rbind(
  continuous.coverage.final %>% mutate(num.Intron = "2 and more"),
  continuous.coverage.final_One_intron %>% mutate(num.Intron = "1"),
  continuous.coverage.final_No_intron %>% mutate(num.Intron = "no")
) %>%
...
Dicistronic.all.tRNA <- continuous.coverage.final.all %>%
  ...
  dplyr::select(id, chr = chr.x, start, end, gene.id = gene.id.x, CDS.start = gene.start, CDS.end = gene.end, gene.strand, tRNA.id, tRNA.start, tRNA.end, tRNA.strand, tRNA.upstream = tRNA.upstream.x, tRNAFirst = tRNAFirst.x, interval.length, continu.cov, num.Intron, combined.continu.cov,
...
DT_Final_Classified <- DT_with_Gene_Startend %>%
  mutate(
    ...
    Location_Type = case_when(
      ...
      tRNA_in_Gene == TRUE & tRNA_in_CDS == FALSE ~ "UTR",
      (tRNA.start < gene.end & tRNA.end > gene.start) & tRNA_in_Gene == FALSE ~ "UTR",
      TRUE ~ "Intergenic"))
```
# export results
```{r}
write.table(DT_Final_Classified,
            file = "Final_Result_withIntronNmb.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
